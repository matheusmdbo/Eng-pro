...
export default function Dashboard({ user, logoutAction, checkoutStatus }: { user: any, logoutAction: any, checkoutStatus?: string }) {
  ...
  const PAYMENT_DISABLED = true; // TEMP: pagamento desativado
  const canAccess = PAYMENT_DISABLED || user.modules.includes(mod);
  const activeModules = PAYMENT_DISABLED ? MODULES : MODULES.filter(m => user.modules.includes(m.id));
  const inactiveModules = PAYMENT_DISABLED ? [] : MODULES.filter(m => !user.modules.includes(m.id));
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'IBM Plex Mono',monospace", fontSize: "12px" }}>
      ...
      <div style={{ display: "flex", minHeight: "calc(100vh - 40px)" }}>
        <div style={{ width: "190px", background: C.s1, borderRight: `1px solid ${C.border}`, padding: "12px", flexShrink: 0, overflowY: "auto" }}>
          ...
          {!PAYMENT_DISABLED && inactiveModules.length > 0 && <><div style={{ fontSize: "8px", color: C.muted, letterSpacing: "2px", margin: "14px 0 8px" }}>BLOQUEADOS</div>{inactiveModules.map(m => (<div key={m.id} onClick={() => setSec("store")} style={{ padding: "7px 10px", marginBottom: "3px", borderRadius: "4px", cursor: "pointer", opacity: 0.4 }}><div style={{ display: "flex", alignItems: "center", gap: "6px" }}><span style={{ fontSize: "12px" }}>🔒</span><div><div style={{ fontSize: "10px", color: C.muted }}>{m.name}</div><div style={{ fontSize: "7px", color: C.muted }}>R$ {m.price.toFixed(0)}</div></div></div></div>))}</>}
        </div>
