...
export default function TranslacaoMod({ onSave, user, UI }: any) {
  ...
  return (<div>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
      <div>
        <h2 style={{ margin: 0, fontSize: "14px", fontWeight: 700 }}>Acionamento da Translação</h2>
        <div style={{ fontSize: "9px", color: C.muted, marginTop: "2px" }}>FEM 2.131/132 · RC-07 — Validado ✓</div>
      </div>
      <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
        <SavedCalcs user={user} moduleType="translacao" onLoad={handleLoad} />
        <button onClick={() => onSave({ type: "translacao", inp, res })} style={sty.btn("g")}>Salvar</button>
        <button onClick={handleCalc} style={sty.btn("p")}>CALCULAR</button>
      </div>
    </div>
    ...
    <div style={{ display: "flex", gap: "4px", marginBottom: "14px", flexWrap: "wrap" }}>
      {subTabs.map((t, i) => (
        <button key={i} onClick={() => setSubTab(i)} style={{
          ...sty.tab(subTab === i), fontSize: "9px", padding: "5px 10px",
        }}>{t}</button>
      ))}
    </div>
